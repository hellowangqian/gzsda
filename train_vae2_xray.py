...
def main(args):
    ...
    vae = VAE2(
        ...
        num_domains = 2,
        num_labels=20 if args.conditional else 0).to(device)    
    ...
    def next_batch(vae,batch_size=64):
        y = np.random.randint(20,size=batch_size)
    ...
    classifier = Classifier(input_dim=2048,num_labels=20).to(device) # train and test a classifier
    ...
    num_epochs = 50
    acc_per_class = np.zeros((num_epochs,20))
